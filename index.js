...
app.use("/api/auth", authRoutes) //登录注册 用户验证
...
app.get('/api/art/:data/:type', (req, res) => {
  ...
  if (type == 0) {
    ...
    //要先通过作者姓名拿到uuid 再在post表中筛选出所搜作者的文章
    const name = req.params.data
    const q = `SELECT uuid FROM user WHERE name=?`
    db.query(q, [name], (err, data0) => {
      if (err) return res.status(500).json(err)
      const user_uuid = data0[0]
      const q = `SELECT * FROM post WHERE user_uuid =?`
      db.query(q, [user_uuid], async (err, data) => {
        if (err) return res.status(500).json(err)
        await Promise.all(data.map(async (item) => {
          const uuid = item.user_uuid
          const q = `SELECT name,major FROM user WHERE uuid=?`
          const newdata = await new Promise((resolve, reject) => {
            db.query(q, [uuid], async (err, data1) => {
              if (err) return res.status(500).json(err)
              const user_name = data1[0].name
              const user_major = data1[0].major
              item['user_name'] = user_name
              item['user_major'] = user_major
              resolve(item)
            })
